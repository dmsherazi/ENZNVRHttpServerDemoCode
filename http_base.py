...
def login_device(save_json_recv):
    ...
    if response.status == 401:
        ...
        # nc and cnonce are randomly generated by the client and need to be included when sending to the server
        nc = "00000002"
        ...
        try:
            ...
            print("The following exception %s" % ex)
            conn.close()
            return -1
        ...
        print(response.status, response.reason)
        print(response.headers)
        # degist During authentication, only the login interface will return S-HASH C-HASH' X-HASH
        # Subsequent operations will need to include these three pieces of information in the http header, otherwise the authentication will not pass
        global s_hash
        global c_hash
        global x_hash
...
#Receive reply Reply in dictionary format dirt = {'recv':''}
def get_recv(dirt):
    global json_recv
    if json_recv == '':
        recv_datas=''
